...
static NSString * const ARGUMENT_WAKEWORD_ENABLED = @"wakeword_enabled";
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result
{
    ...
    {
        [self sendTextWithCall:call result:result];
    }
    ...
    else if( [@"setWakewordEnabled" isEqualToString:call.method] )
    {
        [self setWakewordEnabledWithCall:call result:result];
    }
    else if( [@"getWakewordEnabled" isEqualToString:call.method] )
    {
        [self getWakewordEnabledWithCall:call result:result];
    }
    else
    {
...
- (void)setWakewordEnabledWithCall:(FlutterMethodCall*)call result:(FlutterResult)result
{
    if( self.button == nil )
    {
    ...
    BOOL enabled = [call.arguments[ARGUMENT_WAKEWORD_ENABLED] boolValue];
    [self.button setWakewordEnabled: enabled];
    
    /// Return success result to flutter
    result([NSNumber numberWithBool:YES]);
}

- (void)getWakewordEnabledWithCall:(FlutterMethodCall*)call result:(FlutterResult)result
{
    if( self.button == nil )
    {
        result([FlutterError errorWithCode:@"UNAVAILABLE" message:@"Alan Button unavailable" details:nil]);
    ...
    BOOL enabled = [self.button getWakewordEnabled];
